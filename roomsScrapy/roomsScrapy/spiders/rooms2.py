...
class RoomsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(f'https://www.airbnb.com.co/s/San-Agust%C3%ADn/homes?search_type=filter_change&place_id=ChIJhbXvmZ5wJY4RVAkxc3nRggk&checkin={self.checkin}&checkout={self.checkout}&adults={adults}',
                             callback=self.parse)

        yield scrapy.Request(f'https://www.airbnb.com.co/s/San-Andres--Colombia/homes?tab_id=home_tab&place_id=ChIJabPa3gamBY8RiYudmvoZ3yc&checkin={self.checkin}&checkout={self.checkout}&adults={adults}',
                             callback=self.parse)

        yield scrapy.Request(f'https://www.airbnb.com.co/s/Bogot%C3%A1--Colombia/homes?place_id=ChIJKcumLf2bP44RFDmjIFVjnSM&checkin={self.checkin}&checkout={self.checkout}&adults={adults}',
                             callback=self.parse)

        yield scrapy.Request(f'https://www.airbnb.com.co/s/Santa-Marta-~-Magdalena--Colombia/homes?place_id=ChIJPRea9W_29I4RuPk6FfyVSxI&checkin={self.checkin}&checkout={self.checkout}&adults={adults}',
                             callback=self.parse)
    ...
    def parse(self, response):
        ...
        for room in rooms:
            ...
            precio = int(room.xpath(
                './/div[@class="_ls0e43"]//span[@class="_olc9rf0"]/text()').re(r'^\$(\d+,\d+)')[0].replace(',', ''))
            contains = room.xpath(
